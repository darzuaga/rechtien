...
<div class="entry-meta">
	...
	foreach( $blog_metas as $blog_meta_k => $blog_meta_v ){
		if( $blog_meta_k == 'author' && !empty($blog_meta_v) ){
			echo sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author"><i class="fa fa-user"></i> %3$s</a></span>',
		...
		if( $blog_meta_k == 'categories' && !empty($blog_meta_v) && !empty($category_list) ){
			?>
			<span class="entry-meta-categories"><i class="fa fa-folder-open"></i>&nbsp;<?php echo $category_list;?></span>
			<?php
		}
		
		if( $blog_meta_k == 'tags' && !empty($blog_meta_v) ){
			echo '<span class="entry-meta-tags">'.get_the_tag_list( '<i class="fa fa-tags"></i>', ', ' ).'</span>';
		}
		if( $blog_meta_k == 'date' && !empty($blog_meta_v) ){
			echo sprintf( '<a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s"><i class="fa fa-calendar"></i> %4$s</time></a>',
